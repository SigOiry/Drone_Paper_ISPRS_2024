...
plot_label_Cadiz <- data.frame(x = -6.8,
                         y = 37.15) %>% 
  st_as_sf(coords=c("x","y")) %>% 
  st_set_crs("EPSG:4326")  %>% 
  st_transform(crs(masks_Cadiz)) %>% 
...
ggsave("Figures/High_res/Figure1/Map_Cadiz.png",Cadiz_plot,width= 10, height=10)  
...
plot_label_Tainaron <- data.frame(x = -6.8,
                         y = 37.15) %>% 
  st_as_sf(coords=c("x","y")) %>% 
  st_set_crs("EPSG:4326")  %>% 
  st_transform(crs(masks_Cadiz)) %>% 
...
Tainaron_plot <- ggplot() +
  scale_fill_manual("", labels = c("Land area"),
                    values = c("#CFCFCF"))+
